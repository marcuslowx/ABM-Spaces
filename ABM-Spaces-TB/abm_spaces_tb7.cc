...
struct Simulation {
    ...
    int get_tested(int weeks_infected, int hiv) {
        ...
        // These figures must be updated to take into account evidence on sensitivity based on weeks since TB activation
        if (weeks_infected == 1) {
    ...
    void disease_progress(int num_agents) {
        for (auto& agent: agents) {
            ...
            if ((agent.time_for_test == time_step || agent.time_for_tut == time_step) && agent.tested_positive + agent.dead + agent.dead_natural == 0) {
                int weeks_infected = agent.weeks_exposed + agent.weeks_infectious - tat;
                if (agent.subclinical == 1 || agent.symptomatic == 1) {
                    agent.tested_positive = get_tested(weeks_infected, agent.hiv);
                }
                ++agent.times_tested;
