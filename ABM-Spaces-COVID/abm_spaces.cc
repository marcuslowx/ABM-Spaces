...
struct Parameters {
    ...
    double house_risk = 0.0057;
    double block_risk = 0.0006;
    double class_risk = 0.0025;
    double taxi_risk = 0.08;
    double work_risk = 0.21;
...
struct Simulation {
    ...
    void house_transmit() {
        ...
        for (auto& indices: household_indices) {
            ...
            if (num_infected) {
                double this_house_risk = 1 - pow((1 - house_risk), num_infected);
                for (auto i : house) {
                    if (NOT_INFECTED(agents[i])) {
                        if (rand_range(0, agents.size()) < this_house_risk*agents.size()) {
                            agents[i].exposed = 1;
    ...
    void block_transmit() {
        for (auto& indices: block_indices) {
            ...
            if (current_block.size()) {
                double current_block_risk = 1 - pow((1 - block_risk), infections);
                for (auto i: current_block) {
                    if (NOT_INFECTED(agents[i])) {
                        if (rand_range(0, agents.size()) < current_block_risk*agents.size()) {
                            agents[i].exposed = 1;
    ...
    void class_transmit() {
        for (auto& room_indices: class_indices) {
            ...
            double this_class_risk = 1- pow((1 - class_risk), infected);
            for (auto i: room_indices) {
                if (NOT_INFECTED(agents[i]) &&
                    agents[i].isolated + agents[i].icu  == 0) {
                    if (rand_range(0, agents.size()) < this_class_risk*agents.size()) {
                        agents[i].exposed = 1;
    ...
    void work_transmit() {
        for (auto &indices : work_indices) {
            ...
            double this_workplace_risk = 1- pow((1 - work_risk), infected);
            for (auto i: indices) {
                if (NOT_INFECTED(agents[i]) &&
                    agents[i].isolated + agents[i].icu == 0) {
                    if (rand_range(0, agents.size()) < this_workplace_risk*agents.size()) {
                        agents[i].exposed = 1;
    ...
    void taxi_transmit() {
        ...
        for (int i = 0; i < num_taxis; i++) {
            ...
            if (infections) {
                double current_taxi_risk = 1- pow((1 - taxi_risk), infections);
                for (auto i: taxi) {
                    if (NOT_INFECTED(agents[i])) {
                        if (rand_range(0, agents.size()) < current_taxi_risk*agents.size()) {
                            agents[i].exposed = 1;
